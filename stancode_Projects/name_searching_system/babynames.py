...
def add_file(name_data, filename):
    ...
    """
    with open(filename, 'r') as f: # 固定的寫法：打開 filename 這個檔案，'r' 讀取的意思， as f -> 背起來
        for line in f:
            line = line.split(',')
            if len(line) == 1:
                year = line[0].strip()
...
def search_names(name_data, target):
    ...
    """
    # 將 target 轉為 lowercase
    low_target = target.lower()
    matching_names = []
    for key in name_data:
        # if key.lower() == low_target:
        #     matching_names.append(name_data[low_target])
        if low_target in key.lower():
